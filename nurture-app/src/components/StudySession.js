...
const StudySession = () => {
  ...
  const initializeAWSStrandsSession = async () => {
    ...
    try {
      ...
      if (!initResponse.ok) {
        throw new Error(`Backend session initialization failed: ${initResponse.status}`);
      }
  ...
  const callAWSStrandsAgent = async (agentId, context, studentMessage = "") => {
    ...
    try {
      if (agentGraph && !sessionData.simulationMode) {
        ...
        const messageContent = typeof contentResult.content === 'string' 
  ...
  const determineAgentRouting = (studentMessage) => {
    ...
    if (msg.includes("start") || msg.includes("begin")) {
      return {
        ...
        orchestratorMessage: `🎯 Perfect! Connecting you with the **${agentProfiles[sessionData.sessionPlan?.primaryAgent]?.name}** to begin your ${sessionMode} session.`,
            ...
            <input
              ...
              placeholder={
                sessionStarted
                  ...
                  : "Type 'start' to begin your AWS Strands powered study session..."
              }
